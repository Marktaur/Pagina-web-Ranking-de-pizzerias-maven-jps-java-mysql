...
public class OradoresDAO {
    public void agregarOrador(Orador orador) {
        String sql = "INSERT INTO oradores (nombre, direccion,puntaje, fecha_alta) VALUES (?, ?,?, ?)";
        ...
        try (Connection conn = ConexionDB.conectar();
            ...
            pstmt.setString(2, orador.getDireccion());
             pstmt.setInt(3, orador.getPuntaje());
    ...
    public Orador obtenerPorId(int id) {
        ...
        try (Connection conn = ConexionDB.conectar();
            ...
            if (rs.next()) {
                ...
                orador.setDireccion(rs.getString("direccion"));
                orador.setPuntaje(rs.getInt("puntaje"));
    ...
    public void actualizarOrador(Orador orador) {
        String sql = "UPDATE oradores SET nombre = ?, direccion = ?, puntaje = ?, fecha_alta = ? WHERE id_orador = ?";
        try (Connection conn = ConexionDB.conectar();
             PreparedStatement pstmt = conn.prepareStatement(sql)) {
